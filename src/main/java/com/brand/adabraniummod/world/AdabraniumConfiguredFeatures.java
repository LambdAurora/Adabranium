...
public class AdabraniumConfiguredFeatures {
    ...
    public static void registerAndAddConfiguredFeatures() {
        BuiltinRegistries.CONFIGURED_FEATURE.getKey(VIBRANIUM_ORE).ifPresent(key -> BiomeModifications.addFeature(ctx -> true, GenerationStep.Feature.UNDERGROUND_ORES, key));
        BuiltinRegistries.CONFIGURED_FEATURE.getKey(ADAMANTINE_ORE).ifPresent(key -> BiomeModifications.addFeature(ctx -> true, GenerationStep.Feature.UNDERGROUND_ORES, key));
        BuiltinRegistries.CONFIGURED_FEATURE.getKey(HEART_SHAPED_PLANT).ifPresent(key -> BiomeModifications.addFeature(ctx -> ctx.getBiome().getCategory() == Biome.Category.JUNGLE, GenerationStep.Feature.VEGETAL_DECORATION, key));
    }
}